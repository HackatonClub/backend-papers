...
# 10
```
Как мы видим значение площади не меняется так как она закешировано, чтобы это исправить нужно удалить кеш.
```python
...
```
Первым аргументом является функция, которая будет производить действия над `iterable object`(Итерируемый объект, над которым возможно операция `for i in itrable_object`)
...
# Логируем функцию
```
Это то что когда мы вызываем имя функции и документацию декорированной функции, мы понимаем, что они были заменены значениями из функции декоратора. Это плохо, поэтому есть `wraps`:
```python
